...
class SessionGenerationMixin:
    async def _handle_close(
        ...
        await self.gateway.release_session_lock(session["id"])
        return CommandResult.reply(
            "【开团已关闭】关闭期间不处理消息、不调用模型。"
        )
    async def _handle_finish(
        ...
        await self.gateway.finalize_session(
            session["id"],
            ctx.user_id,
    ...
    def _domain_failure_text(self, action: str, exc: BaseException) -> str:
        return (
            f"【开团】{_OPERATION_LABELS.get(action, '操作')}未执行\n"
            f"原因：{exc}\n"
            "自动处理：系统没有写入任何变更。\n"
            f"下一步：{_DOMAIN_RECOVERY.get(action, '查看 /团 帮助后重试。')}"
        )
