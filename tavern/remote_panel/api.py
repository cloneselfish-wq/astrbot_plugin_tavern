...
class ApiMixin:
    def _api_logout(self) -> None:
        ...
        if token:
            ...
            with self.panel.session_lock:
                ...
                actor = str(entry.get("username") or "") if entry else ""
                self.panel.sessions.pop(digest, None)
        self._audit("", f"panel:{actor or '?'}", "panel.logout", self._client_ip(), {})
        self._send_json(
            {"ok": True},
            headers={
                "Set-Cookie": _panel_cookie(
                    "",
    ...
    def _api_overview(self) -> None:
        ...
        if database is not None:
            rows = self._run_async(database.list_sessions()) or []
            for row in rows:
                if isinstance(row, Mapping):
                    sessions.append(
                        {
                            "name": str(row.get("instance_name") or row.get("name") or ""),
                            "state": str(row.get("state") or ""),
                            "world_name": str(row.get("world_name") or ""),
                        }
                    )
            world_rows = self._run_async(database.list_worlds(False)) or []
        ...
        self._send_json(
            {
                "plugin_version": self.panel.plugin_version,
                "schema_version": self.panel.schema_version,
